...
namespace EnhancedUI.EnhancedScroller
{
    ...
    public class EnhancedScroller : MonoBehaviour
    {
        ...
        public CellViewPositionEnum GetPositionBeforeEnum()
        {
            return CellViewPositionEnum.Before;
        }

        public CellViewPositionEnum GetPositionAfterEnum()
        {
            return CellViewPositionEnum.After;
        }
        ...
        public EnhancedScrollerCellView GetCellView(EnhancedScrollerCellView cellPrefab)
        {
            ...
            if (cellView == null)
            {
                ...
                var go = Instantiate(cellPrefab.gameObject, _container);
        ...
        public void RefreshActiveCellViewAtIndex(int index)
        {
            for (var i = 0; i < _activeCellViews.Count; i++)
            {
                var cellView = _activeCellViews[i];
                if (cellView.dataIndex == index)
                {
        ...
        public void RefreshAndReloadActiveCellViews(bool recycleCells)
        {
            if (recycleCells)
            {
                _RecycleAllCells();
        ...
        public void RefreshAndReloadActiveCellViews(bool recycleCells, float scrollPositionFactor = -1)
        {
            RefreshAndReloadActiveCellViews(recycleCells);
        ...
        public void JumpToDataIndexForce(int dataIndex,
            float scrollerOffset = 0,
            float cellOffset = 0,
            bool useSpacing = true,
            TweenType tweenType = TweenType.immediate,
        ...
        public void SnapTest(int dataIndex)
        {
            if (NumberOfCells == 0) return;
            ...
            // get the data index of the cell at the watch location
            _snapDataIndex = dataIndex; //_snapCellViewIndex % NumberOfCells;
